...
/**
 * Implementation of {@link ibspan.tss.um.ontology.DressSet}
 ...
 * <br>
 */
public class DressSetImpl extends com.ibm.adtech.jastor.ThingImpl implements ibspan.tss.um.ontology.DressSet {
	...
	DressSetImpl(Resource resource, Model model) throws JastorException {
		super(resource, model);
		setupModelListener();
	}     
    	
	static DressSetImpl getDressSet(Resource resource, Model model) throws JastorException {
		if (!model.contains(resource,RDF.type,DressSet.TYPE))
			return null;
		return new DressSetImpl(resource, model);
	}
	    
	static DressSetImpl createDressSet(Resource resource, Model model) throws JastorException {
		DressSetImpl impl = new DressSetImpl(resource, model);
		
		if (!impl._model.contains(new com.hp.hpl.jena.rdf.model.impl.StatementImpl(impl._resource, RDF.type, DressSet.TYPE)))
			impl._model.add(impl._resource, RDF.type, DressSet.TYPE);
		impl.addSuperTypes();
	...
	public java.util.List listStatements() {
		...
		it = _model.listStatements(_resource,RDF.type, ibspan.tss.um.ontology.DressSet.TYPE);
		while (it.hasNext()) {
			list.add(it.next());
		}
	...
	private void initContains() throws JastorException {
		...
		while(it.hasNext()) {
			...
			if (!stmt.getObject().canAs(com.hp.hpl.jena.rdf.model.Resource.class))
				throw new JastorInvalidRDFNodeException (uri() + ": One of the http://www.ibspan.waw.pl/tss/UserModelling#contains properties in DressSet model not a Resource", stmt.getObject());
	...
	public void registerListener(ThingListener listener) {
		if (!(listener instanceof DressSetListener))
			throw new IllegalArgumentException("ThingListener must be instance of DressSetListener"); 
		if (listeners == null)
			setupModelListener();
		if(!this.listeners.contains(listener)){
			this.listeners.add((DressSetListener)listener);
		}
	}
				...
				if (true) { // don't check resource type if the property range is Resource
					...
					if (listeners != null) {
						...
						for(java.util.Iterator iter=consumersForContains.iterator();iter.hasNext();){
							DressSetListener listener=(DressSetListener)iter.next();
							listener.containsAdded(ibspan.tss.um.ontology.DressSetImpl.this,_contains);
						}
					}
				}
				if (_model.contains(resource,RDF.type,ibspan.tss.um.ontology.Ordinal.TYPE)) {
					...
					if (listeners != null) {
						...
						for(java.util.Iterator iter=consumersForContains_asOrdinal.iterator();iter.hasNext();){
							DressSetListener listener=(DressSetListener)iter.next();
							listener.containsAdded(ibspan.tss.um.ontology.DressSetImpl.this,_contains_asOrdinal);
						}
					}
				}
				return;
			...
			if (stmt.getPredicate().equals(containsProperty)) {
				...
				if (true) { // don't check resource type if the property range is Resource
					...
					if (listeners != null) {
						...
						for(java.util.Iterator iter=consumersForContains.iterator();iter.hasNext();){
							DressSetListener listener=(DressSetListener)iter.next();
							listener.containsRemoved(ibspan.tss.um.ontology.DressSetImpl.this,_contains);
						}
					}
				}
				if (_model.contains(resource,RDF.type,ibspan.tss.um.ontology.Ordinal.TYPE)) {
					...
					if (listeners != null) {
						...
						for(java.util.Iterator iter=consumersForContains_asOrdinal.iterator();iter.hasNext();){
							DressSetListener listener=(DressSetListener)iter.next();
							listener.containsRemoved(ibspan.tss.um.ontology.DressSetImpl.this,_contains_asOrdinal);
						}
					}
				}
				return;
